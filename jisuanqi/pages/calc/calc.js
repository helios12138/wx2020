Page({
  ...
  clickBtn:function(event){
    ...
    }else if(id == this.data.idw){
      var pd=this.data.screenData;
    var data=Math.pow(pd,2);
    this.setData({"screenData":data});
    }else if(id==this.data.ids){
      var sd=this.data.screenData;
    var data=Math.pow(sd,3);
    this.setData({"screenData":data});
    }else if(id==this.data.idy){
      var jd=this.data.screenData;
      var result=1;
      for(var i=1;i<=jd;i++){
        result *=i;
      }
      this.setData({"screenData":result});
    }else if(id==this.data.idz){
